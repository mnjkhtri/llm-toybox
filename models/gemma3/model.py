...
class Gemma3MHAttention(nn.Module):
    ...
    def forward(self, resid, past_kv=None, use_cache=False):
        ...
        # expose “Llama-style” score/prob hooks at [B,H,T_new,T_total]
        ...
        if self.is_sliding and K.size(2) > self.window_size:
            K_new = K[:, :, -self.window_size:, :]
            V_new = V[:, :, -self.window_size:, :]
            new_start = end_pos + T_new
            new_end = end_pos + T_new
            return y, PastKV(K_new, V_new, start_pos=new_start, end_pos=new_end)

        return y, PastKV(K, V, start_pos=start_pos, end_pos=end_pos + T_new)

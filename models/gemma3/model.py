...
class Gemma3MHAttention(nn.Module):
    """
    Gemma3 attention (inference):
      - GQA
      - QK-norm
      - RoPE base depends on attention mode:
          local-window: theta=10_000
          global-full:  theta=1_000_000
      - Attention scaling uses query_pre_attn_scalar=256 (hardcoded): scale = 1/sqrt(256)
      - For local-window layers, cache is truncated to last window_size tokens.
    RoPE scaling is OFF.
    """
    def __init__(self, max_length, d_model, n_heads, n_kv_heads, d_head, is_sliding, window_size):
        super().__init__()
        ...
        self.is_local = bool(is_sliding)
        self.attn_mode = "local" if self.is_local else "global"
        ...
        self.rope_theta = 10000.0 if self.is_local else 1_000_000.0
        self.attn_scale = 256.0 ** -0.5 # Gemma3 uses query_pre_attn_scalar=256

    @staticmethod
    def _rope_cos_sin(head_dim, rope_theta, x_pos, device, dtype):
        freq = (1.0 / (2 * math.pi)) * rope_theta ** (-torch.arange(0, head_dim, 2, device=device, dtype=torch.float32) / head_dim)
        phase = torch.einsum("bt,i->bti", x_pos.float(), freq) * (2 * math.pi)  # [B, T, Dh/2] in radians
    ...
    def forward(self, resid, past_kv=None, use_cache=False):
        ...
        q = self.q_proj(resid).view(B, T_new, H, Dh).permute(0, 2, 1, 3)
        k = self.k_proj(resid).view(B, T_new, Hkv, Dh).permute(0, 2, 1, 3)
        v = self.v_proj(resid).view(B, T_new, Hkv, Dh).permute(0, 2, 1, 3)
        ...
        att_g = torch.einsum("bhgqd,bhkd->bhgqk", qg.float(), K.float()) * self.attn_scale

        qpos = end_pos + torch.arange(T_new, device=resid.device).view(T_new, 1)
        kpos = torch.arange(start_pos, start_pos + T_total, device=resid.device).view(1, T_total)
        dist = qpos - kpos

        m = (dist >= 0)
        if self.is_local:
            m = m & (dist < self.window_size)
        m = m.view(1, 1, 1, T_new, T_total)
        ...
        att_g = F.softmax(att_g, dim=-1, dtype=torch.float32).to(q.dtype)
        ...
        ctx = torch.einsum("bhgqk,bhkd->bhgqd", att_g, V).contiguous()
        out = ctx.reshape(B, H, T_new, Dh).permute(0, 2, 1, 3).contiguous()
        ...
        # Keep full cache for global attention; otherwise keep a sliding window.
        keep = min(T_total, self.window_size if self.is_local else T_total)
        cache_K = K[:, :, -keep:, :]
        cache_V = V[:, :, -keep:, :]
        cache_start = new_end_pos - keep

        return y, PastKV(cache_K, cache_V, start_pos=cache_start, end_pos=new_end_pos)


class Gemma3Block(nn.Module):
    def __init__(self, max_length, d_model, d_mlp, n_heads, n_kv_heads, d_head, is_sliding, window_size):
        super().__init__()
        self.mhattn = Gemma3MHAttention(max_length, d_model, n_heads, n_kv_heads, d_head, is_sliding, window_size)
...
class Gemma3(nn.Module, FromPretrainedMixin, InferenceMixin):
    ...
    def __init__(self, vocab_size, max_length, d_model, d_mlp, n_heads, n_kv_heads, d_head, attentions, window_size):
        super().__init__()
        self.vocab_size = vocab_size
        self.max_length = max_length
        self.d_model = d_model
        ...
        self.blocks = nn.ModuleList(
            Gemma3Block(
                max_length, d_model, d_mlp, n_heads, n_kv_heads, d_head,
                is_sliding=(t == 1),
